// Generated by ego on Thu Apr 30 01:41:34 2015.
// DO NOT EDIT
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:22
_, _ = fmt.Fprint(w, "(rows *sql.Rows) (*")
//line model.go.ego:22
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:22
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:34
_, _ = fmt.Fprint(w, " %s\", sqlFieldsFor")
//line model.go.ego:34
_, _ = fmt.Fprintf(w, "%v", m.Name)
//line model.go.ego:34
_, _ = fmt.Fprint(w, "(), cond)\n\trows, err := tx.Query(sql, condFields...)\n\tif err != nil {\n\t\treturn nil, err\n\t}\n\tfor rows.Next() {\n    item, err := load")
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:49
_, _ = fmt.Fprint(w, ") Update(tx dbutil.DBLike) error {\n\t\tstmt, err := tx.Prepare(fmt.Sprintf(\"UPDATE ")
//line model.go.ego:50
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:50
_, _ = fmt.Fprint(w, "(%%s) VALUES(")
//line model.go.ego:50
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:50
_, _ = fmt.Fprint(w, ".ID = ?\", sqlFieldsFor")
//line model.go.ego:50
_, _ = fmt.Fprintf(w, "%v", m.Name)
//line model.go.ego:50
_, _ = fmt.Fprint(w, "()))\n\n\t\tif err != nil {\n\t\t\treturn err\n\t\t}\n\n    params := []interface{}{")
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:67
_, _ = fmt.Fprint(w, ") Insert(tx dbutil.DBLike) error {\n\t\tstmt, err := tx.Prepare(\"INSERT INTO ")
...
_, _ = fmt.Fprintf(w, "%v",  m.Name )
//line model.go.ego:80
_, _ = fmt.Fprint(w, "Table(tx dbutil.DBLike) error {\n\t\tstmt, err := tx.Prepare(`")
